/*
*   문제 : 
        트리의 지름이란, 트리의 임의의 두 노드 중 거리가 가장 먼 노드 간 길이를 의미합니다.
*   설명 : 
        1. 먼저, 임의의 한 정점으로 부터 가장 먼 노드를 구하고, 해당 노드로 부터 한번 더 가장 먼 노드를 구합니다.
        2. 그리고, 문제가 제시하는 트리는 방향성을 갖는 유향 그래프로 가정합니다. 따라서, Parent -> Child + Child -> Parent 방식으로 Edge 목록을 구성할 필요가 없습니다.
